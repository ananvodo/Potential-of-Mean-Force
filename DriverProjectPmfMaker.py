...
try:
    ...
    while pmfSimNum < conf.pmfTotalSteps.value:

        folderName = conf.pmfSimName.value + '_' + str(pmfSimNum) + '_pmf'
        print('\n\n=============================================================')
        print('=============================================================')
        print('\n\nDoing ' + folderName + '\n\n')
        print('=============================================================')
        ...
        groIniName = conf.residToMove.value + str(pmfSimNum) + '.gro'
        groSolvaName = conf.residToMove.value + str(pmfSimNum) + '_solva.gro'
        groIonsName = conf.residToMove.value + str(pmfSimNum) + '_ions.gro'
        tprIniEmName = conf.residToMove.value + str(pmfSimNum) + '_em.tpr'
        groIniEmName = conf.residToMove.value + str(pmfSimNum) + '_em.gro'

        # Names for gro and tpr files inside em_nvt directory (emNvtDir)
        groConcatName = conf.pmfSimName.value + '_' + str(pmfSimNum) + '.gro'
        groConcatEmName = conf.pmfSimName.value + '_' + str(pmfSimNum) + '_em.gro'
        groConcatNvtName = conf.pmfSimName.value + '_' + str(pmfSimNum) + '_nvt.gro'
        tprConcatEmName = conf.pmfSimName.value + '_' + str(pmfSimNum) + '_em.tpr'
        tprConcatNvtName = conf.pmfSimName.value + '_' + str(pmfSimNum) + '_nvt.tpr'

        # Names for tpr file inside pmf directory (pmfDir)
        tprConcatPmfName = conf.pmfSimName.value + '_' + str(pmfSimNum) + '_pmf.tpr'

        dirs = ffg.forMkDirs(mainWorkDir, conf.pmfSimName.value, str(pmfSimNum)) # Creating initial, em_nvt and pmf directories inside current main dir.
        ...
        movedzO = gr.fromList_GROreader_Zmoving(iniBoxOb.file, zDistCumulative, conf.residToMoveIni.value, conf.operation.value)
        ...
        pmfMdpOb.pmfMdpWriter(pmfDir, conf.pmfMdpName.value, zDistCumulative, conf.operation.value)

        # Reading, creating an object, and writing SLURM files in em_nvt and pmf directories
        cpuHungSlurmOb.slurmFileWriter(emNvtDir, conf.cpuHungSlurmName.value, conf.pmfSimName.value+str(pmfSimNum)+'_em', jobSlurmName)
        cpuHungSlurmOb.slurmFileWriter(pmfDir, conf.cpuHungSlurmName.value, conf.pmfSimName.value+str(pmfSimNum)+'_pmf', jobSlurmName)

        gpuHungSlurmOb.slurmFileWriter(emNvtDir, conf.gpuHungSlurmName.value, conf.pmfSimName.value+str(pmfSimNum)+'_em', jobSlurmName)
        gpuHungSlurmOb.slurmFileWriter(pmfDir, conf.gpuHungSlurmName.value, conf.pmfSimName.value+str(pmfSimNum)+'_pmf', jobSlurmName)

        cpuFullnodeSlurmOb.slurmFileWriter(emNvtDir, conf.cpuFullnodeSlurmName.value, conf.pmfSimName.value+str(pmfSimNum)+'_em', jobSlurmName)
        cpuFullnodeSlurmOb.slurmFileWriter(pmfDir, conf.cpuFullnodeSlurmName.value, conf.pmfSimName.value+str(pmfSimNum)+'_pmf', jobSlurmName)
